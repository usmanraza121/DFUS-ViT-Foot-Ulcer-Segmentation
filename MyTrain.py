...
def train(train_loader, model, optimizer, epoch):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            # loss = loss3
            loss = hybrid_e_loss(lateral_map_3, gts)
            # loss = BceDiceLoss()
            # loss = loss(lateral_map_3, gts)
            # ---- backward ----
            loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_path', type=str,
                        default='/media/be-light/Data/PG_Gdansk/conf_paper/experiment/Foot_Ulcer_datasets/FUSeg/train', help='path to train dataset')
    parser.add_argument('--train_save', type=str,
                        default='PolypPVTCFMUpdated1a')
    opt = parser.parse_args()

    # ---- build models ----
    torch.cuda.set_device(0)  # set your gpu device
    model = PolypPVTCFMUpdated().cuda()

    params = model.parameters()
    optimizer = AdaXW(params, opt.lr)

    image_root = '{}/images/'.format(opt.train_path)
    gt_root = '{}/labels/'.format(opt.train_path)
